...
from iliasqc.quiz import create_integrated_quiz_archive
...
def txt_to_quiz_zip(
    ...
    """Convert a question text file to an ILIAS quiz/test zip archive.

    Unlike pools, quiz archives include questions directly embedded in the
    QTI file rather than as pool references. This creates a self-contained
    quiz that can be imported directly into ILIAS.
    ...
    output_path:
        ...
        placed next to the input file with the same stem.
    title:
        Override for the quiz title. If not provided, extracted from the
        ``# TITLE:`` comment in the input file or the filename.
    description:
        Override for the quiz description. If not provided, extracted from
        the ``# DESCRIPTION:`` comment in the input file.
    filter_points:
        If provided, only include questions with this point value.
