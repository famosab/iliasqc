...
def create_manifest(
    ...
    escaped_description = _escape_xml(description)
    ...
    manifest = (
        ...
        f'<Title Language="de">{escaped_title}</Title>'
        '<Language Language="de"/>'
        f'<Description Language="de">{escaped_description}</Description>'
...
def create_manifest_file(
    ...
    escaped_title = _escape_xml_attr(title)

    return (
        '<?xml version="1.0" encoding="utf-8"?>'
        "<!--Generated by ILIAS XmlWriter-->"
        f'<Manifest MainEntity="qpl" Title="{escaped_title}" InstallationId="1600" '
